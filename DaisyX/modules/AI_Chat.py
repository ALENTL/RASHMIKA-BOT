...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Samantha AI Already Activated In This Chat")
            return
        await lel.edit(
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"Samantha AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
@daisyx.on_message(
    filters.regex("Samatha|samantha|shamil|Shamil")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
<b> Assistant </b>
 - /ask <i>question</i>: Ask question from Samantha
